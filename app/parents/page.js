...
import { 
  ArrowLeft, Phone, MessageCircle, MapPin, Clock, Bus, 
  Route, Navigation, MoreVertical, Bell, Info, ChevronUp, ChevronDown
} from "lucide-react";
...
const Tracking = () => {
  ...
  const initializeMap = () => {
    ...
    // Add route line between bus and stop
    drawRouteLine();
  };
  ...
  const drawRouteLine = () => {
    ...
    // Create a polyline between bus and stop
    const routePoints = [
      busLocation,
      [28.9180, 77.1300], // Add a waypoint to make route more realistic
      [28.9160, 77.1330], // Another waypoint
      stopLocation
    ];
  ...
  // Calculate rough distance in km (very simple calculation)
  const calculateDistance = () => {
    const lat1 = busLocation[0];
    const lon1 = busLocation[1];
    const lat2 = stopLocation[0];
    const lon2 = stopLocation[1];
    ...
    <div className="flex flex-col min-h-screen bg-white">
      ...
      <header className="bg-[#FDD51A] px-5 py-3 flex items-center sticky top-0 z-30 shadow-sm">
        ...
        <div className="flex flex-col ml-3">
          <h1 className="font-bold text-gray-900 text-lg">Bus Tracker</h1>
          <p className="text-xs text-gray-700">Real-time location updates</p>
        </div>
        
        <div className="ml-auto flex gap-2">
          <a href="tel:9354855980">
            <button className="bg-white p-2 rounded-full shadow-sm hover:bg-gray-50 transition-colors duration-200" aria-label="Call driver">
              <Phone size={16} className="text-gray-700" />
            </button>
          </a>
          <a href="https://wa.me/+919354855980" target="_blank">
            <button className="bg-white p-2 rounded-full shadow-sm hover:bg-gray-50 transition-colors duration-200" aria-label="Message on WhatsApp">
              <img src="/images/whatsapp.png" alt="WhatsApp" className="w-4 h-4 flex justify-center items-center" />
            </button>
          </a>
          <button 
            className="bg-white p-2 rounded-full shadow-sm hover:bg-gray-50 transition-colors duration-200"
      ...
      {/* Student Info Bar */}
      <div className="bg-white px-5 py-2 flex items-center shadow-sm z-20">
        <div className="bg-white rounded-full h-8 w-8 flex items-center justify-center shadow-sm overflow-hidden">
          <img
        ...
        <div className="ml-auto bg-blue-100 px-3 py-1 rounded-full">
          <span className="text-xs font-medium text-blue-700">Bus #B2854</span>
        </div>
      </div>
      ...
      <div className="relative w-full z-10">
        ...
        <div className="absolute top-4 right-4 flex flex-col gap-2 z-20">
          ...
          >
            {isExpanded ? <ChevronDown size={18} className="text-black" /> : <ChevronUp size={18} className="text-black" />}
          </button>
          <button 
            className="bg-white p-2 rounded-full shadow-md hover:bg-gray-50 transition-colors duration-200"
            onClick={() => {
          ...
          <button 
            className="bg-white p-2 rounded-full shadow-md hover:bg-gray-50 transition-colors duration-200"
            onClick={() => {
              if (leafletMapRef.current && markersRef.current.stop) {
                leafletMapRef.current.setView(stopLocation, 15);
              }
            }}
            aria-label="Center on stop"
          >
