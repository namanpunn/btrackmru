...
function Sidebar({ show, buses, filteredBuses, onSelectBus, onClose }) {
  return (
    <aside className={`fixed inset-y-0 left-0 w-full md:w-80 bg-white shadow-lg transform transition-transform duration-300 z-20 ${show ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 mt-16`}>
      ...
      <div className="hidden md:block p-4 border-b">
        ...
        <p className="text-sm text-gray-600">{filteredBuses.length} of {buses.length} buses</p>
      </div>
...
function BusPopup({ bus }) {
  return (
    <div className="min-w-[200px]">
      <h3 className="font-bold">{bus.name} (#{bus.id})</h3>
      <div className="mt-2 text-sm">
        <p><span className="font-semibold">Status:</span> {bus.status}</p>
        <p><span className="font-semibold">Current Stop:</span> {bus.currentStop}</p>
        <p><span className="font-semibold">Next Stop:</span> {bus.nextStop}</p>
        <p><span className="font-semibold">Occupancy:</span> {bus.occupancy} students</p>
...
export default function AdminBusTracking() {
  ...
  const busIcon = new Icon({
    iconUrl: '/images/bus2.png', // Make sure to add this to your public folder
    iconSize: [32, 32],
    iconAnchor: [16, 32],
    popupAnchor: [0, -32],
  });
  ...
  return (
    <div className="min-h-screen bg-gray-100 relative">
      ...
      <main className="md:ml-80 h-[calc(100vh-4rem)] relative">
